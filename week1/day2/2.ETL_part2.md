![image](https://github.com/ChpcTraining/css2024_notes/assets/157092105/10cb174d-4c35-4b2d-a799-7aad252eb2de)# Extract:
...
One way to make this more efficient is to convert it the lower age in grouping. So instead of 30-34yrs it would 34. This is just one way. You can keep it as is however it will prevent from doing further numerical analysis on it.
...
In pandas, .str, .extract(), and .astype() are all methods that can be applied to a pandas Series object or single column of text data. These methods are part of the powerful functionality provided by pandas for manipulating and analyzing data in a DataFrame. When you apply .str to a column, it allows you to perform various string operations on each element of the Series. Here we used .extract() to extract certain information from a string. Other string methods:
